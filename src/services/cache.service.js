...
class CacheService {
  ...
  async warmCache() {
    ...
    for (const city of this.popularCities) {
      try {
        ...
        const weatherExists = await redisService.exists(this.generateKey('weather', city));
        const forecastExists = await redisService.exists(this.generateKey('forecast', city));
        ...
        logger.error('Error checking cache for city during warming', { city, error: error.message });
        results.push({
          city,
          status: 'error',
          error: error.message
  ...
  async recordCacheHit(city, type) {
    const key = this.generateKey('analytics', 'hits', `${type}:${new Date().toISOString().split('T')[0]}`);
    try {
  ...
  async recordCacheMiss(city, type) {
    const key = this.generateKey('analytics', 'misses', `${type}:${new Date().toISOString().split('T')[0]}`);
    try {
      await redisService.client?.incr(key);
      await redisService.client?.expire(key, 86400 * 7); // Keep for 7 days
    } catch (error) {
  ...
  async getCacheAnalytics(days = 7) {
    ...
    try {
      ...
      for (let i = 0; i < days; i++) {
        ...
        const [weatherHits, weatherMisses, forecastHits, forecastMisses] = await Promise.all([
          redisService.get(this.generateKey('analytics', 'hits', `weather:${dateStr}`)) || 0,
          redisService.get(this.generateKey('analytics', 'misses', `weather:${dateStr}`)) || 0,
          redisService.get(this.generateKey('analytics', 'hits', `forecast:${dateStr}`)) || 0,
          redisService.get(this.generateKey('analytics', 'misses', `forecast:${dateStr}`)) || 0
        ]);
        ...
        analytics.summary.totalHits += analytics.hits[dateStr].weather + analytics.hits[dateStr].forecast;
        analytics.summary.totalMisses += analytics.misses[dateStr].weather + analytics.misses[dateStr].forecast;
      }
      
      const total = analytics.summary.totalHits + analytics.summary.totalMisses;
      analytics.summary.hitRate = total > 0 ? ((analytics.summary.totalHits / total) * 100).toFixed(2) : 0;
      
    } catch (error) {
      logger.error('Error retrieving cache analytics', { error: error.message });
    }
  ...
  async healthCheck() {
    ...
    try {
      ...
      const forecastKeys = await redisService.keys(`${this.prefixes.forecast}*`);
      const analyticsKeys = await redisService.keys(`${this.prefixes.analytics}*`);
  ...
  // Utility methods
  isPopularCity(city) {
    return this.popularCities.some(popular => 
      popular.toLowerCase() === city.toLowerCase()
    );
  }
  ...
  async getCacheInfo(city) {
    ...
    const [weatherTTL, forecastTTL, weatherExists, forecastExists] = await Promise.all([
    ...
    return {
      ...
      weather: {
        ...
        ttl: weatherTTL,
        expiresIn: weatherTTL > 0 ? `${Math.floor(weatherTTL / 60)}m ${weatherTTL % 60}s` : 'N/A'
      },
      forecast: {
        cached: forecastExists,
        ttl: forecastTTL,
        expiresIn: forecastTTL > 0 ? `${Math.floor(forecastTTL / 60)}m ${forecastTTL % 60}s` : 'N/A'
      },
