...
const loadMonitoringConfig = () => {
    ...
    config.ALERT_THRESHOLD_ERROR_RATE = parseInt(config.ALERT_THRESHOLD_ERROR_RATE, 10);
    config.ALERT_THRESHOLD_RESPONSE_TIME = parseInt(config.ALERT_THRESHOLD_RESPONSE_TIME, 10);
    config.HEALTH_CHECK_INTERVAL = parseInt(config.HEALTH_CHECK_INTERVAL, 10);
    config.HEALTH_CHECK_MEMORY_THRESHOLD = parseInt(config.HEALTH_CHECK_MEMORY_THRESHOLD, 10);
    config.HEALTH_CHECK_CPU_THRESHOLD = parseInt(config.HEALTH_CHECK_CPU_THRESHOLD, 10);
    config.PERFORMANCE_METRICS_ENABLED = config.PERFORMANCE_METRICS_ENABLED === 'true';
    config.PERFORMANCE_SAMPLING_RATE = parseFloat(config.PERFORMANCE_SAMPLING_RATE);
    config.SECURITY_LOGGING_ENABLED = config.SECURITY_LOGGING_ENABLED === 'true';
    config.SECURITY_ALERT_THRESHOLD = parseInt(config.SECURITY_ALERT_THRESHOLD, 10);
    config.ADMIN_DASHBOARD_ENABLED = config.ADMIN_DASHBOARD_ENABLED === 'true';

    // Parse histogram buckets
    config.PERFORMANCE_HISTOGRAM_BUCKETS = config.PERFORMANCE_HISTOGRAM_BUCKETS
        .split(',')
        .map(bucket => parseFloat(bucket.trim()));
...
const validateConfig = (config) => {
    ...
    if (!validLogLevels.includes(config.LOG_LEVEL)) {
        throw new Error(`Invalid LOG_LEVEL: ${config.LOG_LEVEL}. Must be one of: ${validLogLevels.join(', ')}`);
    }

    // Validate numeric values
    if (config.METRICS_PORT < 1 || config.METRICS_PORT > 65535) {
        throw new Error(`Invalid METRICS_PORT: ${config.METRICS_PORT}. Must be between 1 and 65535`);
    }

    if (config.PERFORMANCE_SAMPLING_RATE < 0 || config.PERFORMANCE_SAMPLING_RATE > 1) {
        throw new Error(`Invalid PERFORMANCE_SAMPLING_RATE: ${config.PERFORMANCE_SAMPLING_RATE}. Must be between 0 and 1`);
    }

    // Validate paths
    if (!path.isAbsolute(config.LOG_FILE_PATH) && !config.LOG_FILE_PATH.startsWith('./')) {
